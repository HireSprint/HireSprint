...
export const ImageGrid = ({
    ...
    const handleInitChangeProduct = (productId: string): void => {
        const productoSelectIndex = selectedProducts.findIndex((p: Product): boolean => p.id === productId);
        if (productoSelectIndex !== -1) {
            const productoSeleccionado = selectedProducts[productoSelectIndex];
            productoA = productoSeleccionado;  // Actualiza el estado
    ...
    const handleChangeProducts = (cellId: string): void => {
        ...
        if (productoA.gridId !== undefined && productoA.gridId != cellIdNumber) {
            const elementIndex: number = selectedProducts.findIndex((p: Product | null): boolean => p?.gridId === cellIdNumber);

            let productoB: Product;

            if (elementIndex !== -1) {
                // Si se encuentra un producto en el índice, asignarlo a `productoB`
                productoB = selectedProducts[elementIndex]!;
            } else {
                // Si no se encuentra ningún producto, crear un objeto vacío
                productoB = {id: "", image: "", name: "", gridId: cellIdNumber};
            }

            // Intercambiar `gridId` entre `productoA` y `productoB`
            const tempGridId = productoA.gridId;
            productoA.gridId = productoB.gridId;
            productoB.gridId = tempGridId;

            // Actualizar `selectedProducts` con los cambios de `productoA` y `productoB`
            if (elementIndex !== -1) {
                selectedProducts[elementIndex] = productoA; // Reemplaza productoB en la posición `elementIndex`
            }

            const indexA = selectedProducts.findIndex((p: Product): boolean => p?.id === productoA.id);
            if (indexA !== -1) {
                selectedProducts[indexA] = productoB; // Coloca productoA en la posición de `productoB`
            }

            console.log('Producto A después del intercambio:', productoA);
            console.log('Producto B después del intercambio:', productoB);

            // Resetear productoA y productoB si es necesario
            resetProductoA();
            resetProductoB();
        }
    };
    ...
    export const ImageGrid2 = ({
        ...
        const handleChangeProducts = (cellId: string): void => {
            ...
            products.length = 0;
            console.log(selectedProducts + " elemento añadidos");
        };
